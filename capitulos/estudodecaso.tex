...
Foram selecionadas algumas funcionalidades muito comuns em aplicativos para serem comparadas tecnicamente entre iOS, Android e Ionic, com o objetivo de melhorar o embasamento na tomada de decisão entre o desenvolvimento 
nativo e multiplataforma. São apresentados trechos de códigos minerados de repositórios públicos e criados pelos autores, comentados para facilitar o entendimento das funcionalidades.
...
Firebase\footnote{\url{https://www.firebase.com}} do Google e o \textit{plugin phonegap-facebook-plugin}\footnote{\url{https://github.com/Wizcorp/phonegap-facebook-plugin.git}}.
Desta forma, é possível acessar os dados públicos do usuário como nome, e-mail e foto de perfil para realizar um cadastro e \textit{login} no aplicativo.
...
obtidos, o aplicativo se encarrega de realizar o cadastro e \textit{login} do usuário da forma como melhor convier. A seguir são apresentados dois trechos de código no iOS e no Ionic para \textit{login} com o Facebook. 
No iOS foi utilizado o \textit{SDK} nativo do Facebook, já no Ionic, foi utilizado o \textit{Firebase} 

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/ios/loginfb}
	\caption[Trecho de código realizando requisição de dados do Facebook - iOS]{Trecho de código realizando requisição de dados do Facebook - iOS}
...
\begin{figure}[H]
	...
	\caption[Trecho de código realizando requisição de dados do Facebook - Ionic]{Trecho de código realizando requisição de dados do Facebook - Ionic}
	\label{fig:loginfb-ios}
\end{figure}
...
perdas de dados no caso de roubos, extravios ou danos ao aparelho, facilitar a migração para outros dispositivos e uso em outras plataformas. Com isso, é necessário que os aplicativos estejam preparados para se 
...
e utilizar os dados de acordo com as necessidades do aplicativo. Um trecho de código exemplificando o uso do \textit{AlamoFire} e \textit{SwiftyJSON} é apresentado a seguir.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/ios/alamofire}
	\caption[Trecho de código utilizando Alamofire e SwiftyJSON - iOS]{Trecho de código utilizando Alamofire e SwiftyJSON - iOS}
...
resposta, para só então ser criada uma requisição que permite o envio de dados. No exemplo a seguir a classe criada chama-se \textit{CustomJSONObjectRequest} e sua implementação não se encontra na imagem. 

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/android/volley}
	\caption[Trecho de código utilizando Volley - Android]{Trecho de código utilizando Volley - Android}
...
Em alguns casos, pode ser necessário a extração de metadados de algum arquivo específico, como uma imagem ou um áudio. No caso de uma imagem, 
podem ser retiradas informações como local, hora de criação e até mesmo dados climáticos no momento da captura da foto. No caso de um arquivo de áudio, é possível extrair o título do áudio, o autor ou artista, 
tempo de duração e álbum. Esses metadados podem ser usados para fornecer mais informações pertinentes ao usuário sobre aquilo que ele está usando, ou até mesmo para fazer filtros e buscas mais inteligentes nos dados que o aplicativo manuseia. 
...
\begin{figure}[H]
	...
	\caption[Trecho de código para extrair metadados de áudio - iOS]{Trecho de código para extrair metadados de áudio - iOS}
...
\begin{figure}[H]
	...
	\caption[Trecho de código para extrair metadados de mídia - Android]{Trecho de código para extrair metadados de mídia - Android}
...
Vale ressaltar que o Android já possui a capacidade de ler a área de toque por meio de \textit{software}, no entanto, o \textit{Force Touch} não é apenas a leitura de área de toque, mas sim da pressão do toque,
o que envolve \textit{hardware} e \textit{software}.
A seguir são apresentados trechos de código iOS e Ionic para utilização do \textit{3DTouch}.
%https://github.com/versluis/3D-Touch
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/ios/3dtouch}
	\caption[Trecho de código implementando 3DTouch no iOS]{Trecho de código implementando 3DTouch no iOS}
	\label{fig:3dtouch-ios}
\end{figure}
%https://github.com/EddyVerbruggen/cordova-plugin-3dtouch
%https://github.com/ashteya/ionic-tutorial-quickactions/blob/master/www/js/quickaction.service.js
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/ionic/forcetouch}
	\caption[Trecho de código implementando 3DTouch no Ionic]{Trecho de código implementando 3DTouch no Ionic}
	\label{fig:forcetouch-ionic}
\end{figure}

\subsubsection{\textit{Smartwatches}} \label{subsubsec:facial} 

Além de \textit{smartphones} e \textit{tablets}, estão começando a ficar populares os dispositivos \textit{wearables} ou \textit{vestíveis} como relógios inteligentes. Isso implica em mais plataformas para dominar 
e mais possibilidades de aplicativos e funcionalidades para desenvolver. Um dos \textit{smartwatches} mais populares é o Apple Watch. Lançado com um sistema operacional próprio derivado do iOS, o WatchOS, 
...
Caso haja a necessidade de criar um aplicativo que suporte o Apple Watch, pode-se optar por desenvolver multiplataforma também, pois já existem \texit{plugins}
\footnote{\url{https://github.com/leecrossley/cordova-plugin-apple-watch}} 
do Cordova para realizar a integração entre o aplicativo multiplataforma e a extensão para Apple Watch. No entanto, o \texit{plugin} citado só funciona enquanto o aplicativo no iPhone está rodando. Quando, por algum motivo, 
o aplicativo deixa de rodar o \texit{plugin} não consegue mais se comunicar com o relógio podendo prejudicar a experiência de uso do aplicativo. Com isso, pode não ser uma boa ideia utilizar funcionalidades que 
...
Vale ressaltar que o aplicativo no Apple Watch deve ser feito nativamente, diretamente no Xcode! O \textit{plugin} apenas fornece um meio para que o \textit{app} do iPhone possa transmitir e receber dados do \textit{app}
...
deve ser feito o aplicativo usando Cordova e Ionic para o iPhone, depois criar o \textit{app} no Xcode para Apple Watch e então utilizar um \textit{plugin} para realizar a comunicação entre os dois aplicativos.  


Tbm é possivel criar a


ja tenho um app multiplataforma e agora quero dar suporte ao apple watch, é possivel

mas se o objetivo for o apple watch ainda eh melhor fazer nativamente 
eh legal fazer no multiplataforma pq ainda assim ja teria o app para ios e android e ainda poderia fazer o do relogio
MAS a comunicacao entre o iPhone e o relogio nao eh perfeita no multiplataforma como eh no nativo, pois o app precisa estar rodando para a comunicacao funcionar corretamente

\subsubsection{Detecção facial} \label{subsubsec:facial}
