...
perdas de dados no caso de roubos, extravios e danos ao aparelho, facilitar a migração para outros dispositivos e uso em outras plataformas. Com isso, é necessário que os aplicativos estejam preparados para se 
...
são muito utilizados e conhecidos na comunidade de desenvolvedores tanto para comunicar com as \textit{APIs}, como para lidar com JSON e XML usados na comunicação entre aplicativo/servidor. 
...
\textit{frameworks} terceiros como \textit{AlamoFire}\footnote{\url{https://github.com/Alamofire/Alamofire}} que abstraem essa comunição e tratamento dos dados tornando o código mais simples e limpo. Ao utilizá-lo,
o programador apenas indica a \textit{URL}, o método da requisição (\textit{POST, GET, PUT, DELETE, etc}) e os parâmetros que deseja enviar ao servidor. Após isso, basta receber a resposta na forma de \textit{JSON},
e utilizar de acordo com as necessidades do aplicativo. Um trecho de código exemplificando o uso do \textit{AlamoFire} é apresentado a seguir.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/ios/alamofire}
	\caption[Trecho de código utilizando Alamofire - iOS]{Trecho de código utilizando Alamofire - iOS}
