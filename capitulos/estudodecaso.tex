...
A seguir são apresentados trechos de códigos que implementam a verificação biométrica em iOS e Android.
...
cada plataforma a autenticação se dá de maneira específica e ainda não há um \textit{plugin} para ambas as plataformas. A seguir são apresentados dois trechos de códigos no Ionic implementando 
a leitura biométrica. Foram usados os seguintes \textit{plugins}, para iOS e Android, respectivamente:
\begin{itemize}
	\item \textit{cordova-plugin-touchid}\footnote{\url{https://github.com/leecrossley/cordova-plugin-touchid}};
	\item \textit{cordova-plugin-android-fingerprint-auth}\footnote{\url{https://github.com/mjwheatley/cordova-plugin-android-fingerprint-auth}}; 
\end{itemize}
 
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/ionic/touchid}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{codes/ionic/fingerprint}
...
\begin{figure}[H]
	...
	\caption[Trecho de código para extrair metadados de mídia - Ionic]{Trecho de código para extrair metadados de mídia - Ionic}
...
com diversas possibilidades de aplicações, tais como reconhecimento facial e videoconferências \cite{wong_efficient_2000}.
...
a aplicações iOS nativas e a aplicações multiplataformas desenvolvidas com o uso de actionscript \cite{site_beyond_2016}.
...
O Scanbot é um exemplo de SDK que pode ser utilizado junto ao cordova e possibilita detecção de documentos e de faces em tempo real,
 porém não é totalmente compatível com iOS \cite{gmbh_javascript_2016} e não é uma ferramenta gratuita.
