...
Cross: Acesso limitado a funcionalidades, 
...
\subsection{Desenvolvimento \textit{iOS}} \label{arc-ios-section}
...
Cada plataforma possui seu próprio ambiente de desenvolvimento, arquitetura, ferramentas, mercado de distribuição e público alvo. %(COMPONENT BASED FRAMEWORK TO CREATE MOBILE CROSS-PLATFORM APPLICATIONS, 2013)
E cada abordagem de desenvolvimento possui suas características próprias e inerentes à forma como se dá o projeto, desenvolvimento e distribuição dos \textit{apps}.
...
da aplicação e por consequência os lucros obtidos com o \textit{app}.
...
pois para cada plataforma há uma arquitetura distinta que deve ser seguida, padrões de código e interface de usuário e \textit{API's} próprias. 
...
para ser a mesma aplicação em plataformas distintas e por fim, gerados novos executáveis e distribuídos nas diferentes lojas de aplicativos.

Isso implica em planejar cada \textit{app} com base em uma plataforma distinta, por mais
que se trate do mesmo aplicativo, o que acaba por encarecer e tornar mais lento o desenvolvimento. 
Por exemplo, existem componentes que não existem em todas as arquiteturas 
(\textit{navigationController} e \textit{tabBarController}, por exemplo) e que devem ser considerados no projeto do \textit{app} para as demais plataformas.
...
apenas em uma arquitetura, um ambiente de desenvolvimento, um conjunto único de ferramentas e não de acordo com as características de cada plataforma. 
Dessa forma, a equipe de desenvolvedores pode focar nos requisitos da aplicação sem se preocupar tanto com as diferenças entre 
plataformas.
...
\begin{figure}[H]
  \centering
    \includegraphics[width=1.0\textwidth]{dev-natxcross}%potential advantages and disadivantages of multiplataform development frameworks/shakshuki et al.
    \caption[Desenvolvimento \textit{Cross-plataform} x Nativo]{ Desenvolvimento \textit{Cross-plataform} x Nativo. Fonte: Baseado em shakshuki et al.}
...
\begin{table}[h]
	...
	\begin{tabular}{ccc}
		\toprule
		\textbf{} & \textbf{Vantagens} & \textbf{Desvantagens} \\
		\midrule
		\multirow{3}{*}{Nativo}             & Explora todas as capacidades do dispositivos                                                                          & Necessário um aplicativo por plataforma \\
		                                    & Maior performance                                                                                                     & Mais caro e mais difícil de manter, por ter que manter varios apps diferentes \\
                                            & Oferece experiencia nativa de usabilidade para o usuario                                                              & Requer domínio de vários ambientes e linguagens para cada plataforma \\
        \multirow{4}{*}{Multiplataformas}   & Só é preciso dominar uma linguagem e um ambiente                                                                      & Não possui acesso a todas as funcionalidades do dispositivo \\
                                            & Desenvolve apenas um código e pode distribuir em várias lojas de apps, o que faz com que o alcance do app seja maior  & Menor performance comparada ao nativo, linguagens interpretadas e compiladas \\
                                            & Reduz custo, tempo e esforço de desenvolvimento e manutenção                                                          & Não possui a mesma usabilidade e experiência de uso das aplicações nativas \\
                                            & & Não possui as últimas atualizações lançadas pelo SO, pois para cada atualização do SO, é preciso ser feita uma atualização na plataforma cross \\
		\bottomrule
	\end{tabular}
	\caption{Vantagens x Desvantagens de desenvolvimento multiplataformas em relação ao nativo}
\end{table}
\end{comment}