...
Segundo \citeonline{kassas_taxonomy_2015}, o desenvolvimento de aplicativos móveis diferencia-se do desenvolvimento de outros tipos de \textit{software} por possuir particularidades e restrições. Os desenvolvedores devem 
...
De acordo com \citeonline{corral_ant_2012}, aplicações móveis  são desenvolvidas dinamicamente e lançadas no mercado em pequenos ciclos. Os produtos finais costumam ser de pequeno porte e comercializados a preços baixos. 
As equipes de desenvolvimento também tendem a ser pequenas. Algumas particularidades a serem consideradas no desenvolvimento \textit{mobile} são listadas a seguir, baseadas em \citeonline{kassas_taxonomy_2015}.
...
A maioria das interfaces disponíveis para uso são disponibilizadas por meio de \textit{frameworks} que podem ser adicionados ao projeto no \textit{Xcode}, contendo \textit{DSLs (Dynamic Shared Libraries)} e os recursos necessários como, 
...
\begin{itemize}
    ...
    com o usuário como entrada baseada em toques, notificações \textit{Push} e outras tecnologias necessárias para
     ...
     de alto nível que permitem acesso à funcionalidades do sistema como \textit{AddressBook} para contatos, \textit{EventKit} 
...
%Quando se desenvolve, nativamente, um aplicativo, é necessário perpassar todo o processo de desenvolvimento novamente para cada plataforma alvo que se deseja alcançar.
...
Conforme \citeonline{heitkotter_evaluating_2013}, essas aplicações são comumente categorizadas em \textit{webapps} e aplicações híbridas.
...
como tamanho de tela e modo de uso \cite{heitkotter_evaluating_2013}. Segundo \citeonline{stark_building_2010}, \textit{web apps} não são instalados 
...
Conforme \citeonline{heitkotter_evaluating_2013}, \textit{web apps} costumam ter a aparência e o comportamento de páginas \textit{web} comuns, 
divergindo dos elementos de tela padrão providos pelas plataformas \textit{mobile}.

A abordagem híbrida surgiu como uma combinação do uso das tecnologias \textit{web} junto à possibilidade de acesso a funcionalidades nativas \cite{heitkotter_evaluating_2013}.
...
para garantir que outras empresas pudessem contribuir \cite{bezerra_desenvolvimento_2016}.%, já que muitas empresas já conheciam as licenças da Apache \cite{bezerra_desenvolvimento_2016}.
...
Cordova permite que aplicações não nativas tenham acesso à funcionalidades nativas do dispositivo como sensores, 
...
%NESTA SEÇÃO e do \textit{Ionic Framework}, usado para desenvolvimento multiplataforma e \textit{frameworks} necessários para o correto funcionamento do Ionic...
